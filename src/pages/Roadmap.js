...
const difficultyColor = { Beginner: "#34d399", Intermediate: "#fbbf24", Advanced: "#f43f5e" };
      ...
      <div className="glass hover:border-white/15 transition-all duration-300 overflow-hidden"
        ...
        <button className="w-full text-left p-5" onClick={() => setOpen(!open)}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  <h3 className="text-white font-bold" style={{ fontFamily: "Space Grotesk" }}>{phase.title}</h3>
                  <span className="badge text-xs" style={{
                    background: `rgba(${phase.difficulty === "Beginner" ? "52,211,153" : phase.difficulty === "Intermediate" ? "251,191,36" : "248,113,113"},0.1)`,
                    color: difficultyColor[phase.difficulty] || "#71717a",
                    border: `1px solid ${difficultyColor[phase.difficulty] || "#71717a"}33`,
            ...
            <div className="flex items-center gap-3">
              <span className="text-[#52525b] text-xs hidden md:block">{phase.topics?.length} topics</span>
          ...
          {/* Objective */}
          {!open && (
            <p className="text-[#71717a] text-sm mt-3 ml-14">{phase.objective}</p>
          )}
        </button>
        ...
        {open && (
          <div className="px-5 pb-5 ml-14 space-y-5 animate-fade-in">
            <p className="text-[#71717a] text-sm">{phase.objective}</p>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {phase.topics?.map((t, i) => (
                  <span key={i} className="glass px-3 py-1.5 text-xs text-[#71717a] rounded-xl hover:text-white transition-colors">{t}</span>
                ))}
              </div>
            </div>
                ...
                <div className="grid md:grid-cols-2 gap-2">
                  {phase.resources.map((r, i) => (
                    <div key={i} className="flex items-center gap-2.5 p-3 rounded-xl border border-white/7 hover:border-white/15 transition-all">
                      <span className="text-[#71717a]">{resourceIcon[r.type] || <BookOpen size={13} />}</span>
                      <div className="min-w-0">
                        <p className="text-white text-xs font-medium truncate">{r.name}</p>
                        {r.author && <p className="text-[#52525b] text-xs">by {r.author}</p>}
                        {r.url && r.url !== "url" && <p className="text-[#52525b] text-xs truncate">{r.url}</p>}
                      </div>
                    </div>
                  ))}
                </div>
...
const Roadmap = () => {
  ...
  const phaseColors = ["#f59e0b", "#10b981", "#8b5cf6", "#34d399"];
      ...
      <div className="main-content">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="mb-6 animate-fade-up">
            ...
            <p className="text-[#71717a] mt-1">Get a step-by-step learning path personalized for your goal</p>
          </div>
            ...
            {[
              ...
              <button key={t.id} onClick={() => setActiveTab(t.id)}
                className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all ${activeTab === t.id
                    ? "bg-[#f59e0b]/15 text-[#fbbf24] border border-[#f59e0b]/20"
          ...
          {activeTab === "saved" && (
            <div className="animate-fade-up">
              {loadingSaved ? (
                <div className="flex justify-center py-12"><Loader size={24} className="animate-spin text-[#f59e0b]" /></div>
              ) : savedRoadmaps.length === 0 ? (
                <div className="glass-bright p-12 text-center">
                  <p className="text-4xl mb-3">🗺️</p>
                  <p className="text-white font-semibold">No roadmaps yet</p>
                  <p className="text-[#71717a] text-sm mt-1">Generate your first roadmap above</p>
                </div>
              ) : (
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <div className="flex gap-2 mt-1">
                            ...
                            <span className="badge text-xs" style={{ background: "rgba(255,255,255,0.05)", color: "#71717a" }}>{r.currentLevel}</span>
                          </div>
                        </div>
                        <ArrowRight size={16} className="text-[#52525b]" />
                      </div>
                    </button>
                  ))}
                </div>
            ...
            <>
                ...
                <div className="animate-fade-up">
                  <div className="glass-bright p-6">
                    <form onSubmit={handleGenerate} className="space-y-5">
                      ...
                      {/* Goal */}
                      <div>
                        <label className="block text-sm text-[#71717a] mb-2">What is your goal?</label>
                          ...
                          {GOALS.map(g => (
                            <button key={g} type="button" onClick={() => setForm({ ...form, goal: g })}
                              className={`px-3 py-2.5 rounded-xl text-xs font-medium text-left transition-all ${form.goal === g
                                  ...
                                  : "border border-white/7 text-[#71717a] hover:border-white/15 hover:text-white"
                                }`}>
                              {g}
                            </button>
                          ))}
                      ...
                      {/* Focus Area */}
                      <div>
                        <label className="block text-sm text-[#71717a] mb-2">Focus Area</label>
                        <div className="flex flex-wrap gap-2">
                          {FOCUS_AREAS.map(f => (
                            <button key={f} type="button" onClick={() => setForm({ ...form, focusArea: f })}
                              className={`px-3 py-1.5 rounded-xl text-xs font-medium transition-all ${form.focusArea === f
                                  ...
                                  : "border border-white/7 text-[#71717a] hover:border-white/15"
                                }`}>
                              {f}
                            </button>
                          ))}
                      ...
                      <div className="grid md:grid-cols-2 gap-4">
                        ...
                        <div>
                          <label className="block text-sm text-[#71717a] mb-2">Timeframe</label>
                      ...
                      <div>
                        <label className="block text-sm text-[#71717a] mb-2">Current Skills (optional)</label>
              ...
              {step === "loading" && (
                <div className="glass-bright p-16 text-center animate-fade-in">
                  <div className="w-16 h-16 rounded-2xl bg-[#f59e0b]/15 flex items-center justify-center mx-auto mb-5 animate-float">
                    <Map size={30} className="text-[#fbbf24]" />
                  </div>
                  <h2 className="text-xl font-bold text-white mb-2" style={{ fontFamily: "Space Grotesk" }}>
                    AI is building your roadmap...
                  </h2>
                  <p className="text-[#71717a] text-sm mb-6">Crafting a personalized path for your goal</p>
                  <div className="flex justify-center gap-2">
                    {["Analyzing goal", "Building phases", "Adding resources", "Finalizing"].map((s, i) => (
                      <div key={i} className="flex items-center gap-1.5">
                        <div className="w-1.5 h-1.5 rounded-full bg-[#f59e0b] animate-pulse" style={{ animationDelay: `${i * 0.3}s` }} />
                ...
                <div className="animate-fade-up space-y-5">
                  ...
                  <div className="relative overflow-hidden rounded-2xl p-6 border border-[#f59e0b]/20"
                    ...
                    <div className="absolute top-0 right-0 w-48 h-48 rounded-full opacity-10 blur-3xl pointer-events-none"
                      style={{ background: "radial-gradient(circle, #f59e0b, transparent)", transform: "translate(20px,-20px)" }} />
                    ...
                    <div className="relative z-10">
                      ...
                      <p className="text-[#71717a] text-sm leading-relaxed mb-4">{roadmap.overview}</p>
                      ...
                      <div className="flex flex-wrap gap-3">
                        ...
                        {roadmap.careerOutcome && (
                          <div className="flex items-center gap-1.5 text-sm">
                            ...
                            <span className="text-[#71717a] text-xs">{roadmap.careerOutcome}</span>
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  {roadmap.keySkills?.length > 0 && (
                    <div className="glass p-5">
                      <h3 className="text-white font-bold mb-3 flex items-center gap-2" style={{ fontFamily: "Space Grotesk" }}>
                        <Zap size={16} className="text-[#fbbf24]" /> Key Skills You'll Master
                      </h3>
                      <div className="flex flex-wrap gap-2">
                  ...
                  <div>
                    <h3 className="text-white font-bold mb-4 flex items-center gap-2" style={{ fontFamily: "Space Grotesk" }}>
                      <Map size={16} className="text-[#10b981]" /> Your Learning Path
                    </h3>
                    <div className="space-y-3">
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    {roadmap.dailySchedule && (
                      <div className="glass p-5">
                        <h3 className="text-white font-bold mb-3 flex items-center gap-2" style={{ fontFamily: "Space Grotesk" }}>
                          <Calendar size={16} className="text-[#8b5cf6]" /> Daily Schedule
                        </h3>
                        <div className="space-y-2">
                          <div className="p-3 rounded-xl border border-white/7">
                            <p className="text-xs text-[#71717a] mb-1">Weekdays</p>
                            <p className="text-white text-sm">{roadmap.dailySchedule.weekday}</p>
                          </div>
                          <div className="p-3 rounded-xl border border-white/7">
                            <p className="text-xs text-[#71717a] mb-1">Weekends</p>
                        ...
                        <div className="space-y-2">
                          {roadmap.tips.map((tip, i) => (
                            <div key={i} className="flex items-start gap-2">
                              ...
                              <p className="text-[#71717a] text-sm">{tip}</p>
                            </div>
                          ))}
                        </div>
                      </div>
