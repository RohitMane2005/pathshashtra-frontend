        ...
        <div className="max-w-3xl mx-auto">
          ...
          {step === "loading" && (
            <div className="flex justify-center items-center h-64">
              <Loader size={32} className="animate-spin text-[#00D4C8]" />
            </div>
          )}
          ...
          {step === "setup" && (
            <div className="animate-fade-up">
              <div className="text-center mb-8">
                <div className="w-14 h-14 rounded-2xl bg-[#00D4C8]/10 border border-[#00D4C8]/20 flex items-center justify-center mx-auto mb-4 animate-float">
                  <BookOpen size={26} className="text-[#00D4C8]" />
                </div>
                <h1 className="text-3xl font-bold text-white mb-2" style={{ fontFamily: "Bricolage Grotesque" }}>Create Study Plan</h1>
                <p className="text-[#7A7890]">AI will build your personalized exam schedule</p>
              </div>
              ...
              <div className="glass-bright p-6">
                <form onSubmit={generatePlan} className="space-y-5">
                  <div className="grid md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm text-[#7A7890] mb-2">Plan Title</label>
                    ...
                    <div>
                      <label className="block text-sm text-[#7A7890] mb-2">Exam Date</label>
                    ...
                    <div>
                      <label className="block text-sm text-[#7A7890] mb-2">Exam Type</label>
                    ...
                    <div>
                      <label className="block text-sm text-[#7A7890] mb-2">Daily Study Hours</label>
                    ...
                    <div>
                      <label className="block text-sm text-[#7A7890] mb-2">Subjects (comma-separated)</label>
          ...
          {step === "plan" && (
            <div className="animate-fade-up space-y-4">
              ...
              <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                  <h1 className="text-2xl font-bold text-white" style={{ fontFamily: "Bricolage Grotesque" }}>{plan?.planTitle}</h1>
                  <p className="text-[#7A7890] text-sm flex items-center gap-2 mt-1">
                ...
                <div className="glass-bright p-5">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-white font-bold flex items-center gap-2" style={{ fontFamily: "Bricolage Grotesque" }}>
                      <TrendingUp size={16} className="text-[#00D4C8]" /> Overall Progress
                    </h3>
                    <span className="text-3xl font-bold text-[#00D4C8]" style={{ fontFamily: "Bricolage Grotesque" }}>
                  ...
                  <div className="progress-bar mb-3" style={{ height: "6px" }}>
                    <div className="h-full rounded-full transition-all" style={{
                      ...
                      background: "linear-gradient(90deg, #00D4C8, #00F5E9)"
                    }} />
                  </div>
                  <div className="flex items-center justify-between text-xs text-[#7A7890]">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-4">
                      {progress.subjectProgress.map((s, i) => (
                        <div key={i} className="glass p-3 text-center">
                          <p className="text-[#00D4C8] font-bold text-lg">{s.percent}%</p>
                          <p className="text-[#7A7890] text-xs truncate">{s.subject}</p>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                {[
                  ...
                  <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                    className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all flex items-center gap-2 ${
                      activeTab === tab.id
                        ? "bg-[#00D4C8]/15 text-[#00D4C8] border border-[#00D4C8]/20"
                        : "text-[#7A7890] border border-white/7 hover:border-white/15"
                    }`}>
                    {tab.label}
                    <span className={`w-5 h-5 rounded-full text-xs flex items-center justify-center font-bold ${
                      activeTab === tab.id ? "bg-[#00D4C8]/20 text-[#00D4C8]" : "bg-white/5 text-[#3D3B52]"}`}>
                      {tab.count}
                    </span>
                  </button>
                ))}
                ...
                <div className="glass p-5">
                  {todayTopics.length === 0 ? (
                    <div className="text-center py-10">
                      ...
                      <p className="text-[#7A7890] text-sm mt-1">Come back tomorrow for new topics</p>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      {todayTopics.map((topic, i) => (
                        <div key={i} className={`flex items-center justify-between p-4 rounded-xl border transition-all ${
                          ...
                          <div>
                            <p className={`font-semibold text-sm ${topic.status === "COMPLETED" ? "text-[#7A7890] line-through" : "text-white"}`}>
                              {topic.topicName}
                            </p>
                ...
                <div className="glass p-5">
                  {weakTopics.length === 0 ? (
                    <div className="text-center py-10">
                      ...
                      <p className="text-[#7A7890] text-sm mt-1">Your understanding is solid across all subjects</p>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      {weakTopics.map((topic, i) => (
                        <div key={i} className="flex items-center justify-between p-4 rounded-xl border border-yellow-400/20 bg-yellow-400/5">
                          <div>
                            ...
                            <p className="text-[#7A7890] text-xs mt-0.5">{topic.subject} · Confidence: {topic.confidenceScore}/10</p>
                          </div>
